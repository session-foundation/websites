...
export function StakedNodesWithAddress({ address }: { address: Address }) {
  ...
  const { setNetworkStatusVisible } = useNetworkStatus({
    network,
    isLoading,
    isFetching,
    refetch,
...
function NoNodes() {
  const dictionary = useTranslations(
    useActiveVestingContract()
      ? "vesting.modules.stakes"
      : "modules.stakedNodes"
  );
  return (
    <ModuleGridInfoContent>
      ...
      <Link
        href="/stake"
        prefetch
        data-testid={LinkDataTestId.No_Nodes_Stake_Now}
      >
        <Button
