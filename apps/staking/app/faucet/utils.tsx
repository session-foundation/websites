...
export const setupDatababse = () => {
  ...
  db.prepare(
    `CREATE INDEX IF NOT EXISTS operator_index_wallet ON ${TABLE.TRANSACTIONS} (${TRANSACTIONS_TABLE.WALLET}, ${TRANSACTIONS_TABLE.TIMESTAMP}) WHERE ${TRANSACTIONS_TABLE.WALLET} IS NOT NULL;`
  ).run();

  db.prepare(
    `CREATE INDEX IF NOT EXISTS operator_index_code ON ${TABLE.TRANSACTIONS} (${TRANSACTIONS_TABLE.TARGET}, ${TRANSACTIONS_TABLE.TIMESTAMP}, ${TRANSACTIONS_TABLE.CODE}) WHERE ${TRANSACTIONS_TABLE.CODE} IS NOT NULL;`
  ).run();
...
type CreateWalletReferralCodesParams = {
  params: Array<AddReferralCodesParams>;
};

export function createWalletReferralCodes({ params }: CreateWalletReferralCodesParams) {
  try {
    for (const { codes, maxUses, drip, creatorWallet } of params) {
      addReferralCodes({ codes, maxUses, drip, creatorWallet });
    }
  } catch (error) {
    console.error('Failed to creat referral codes', error);
  }
